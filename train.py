...
# parser.add_argument('--numepochs', type=int, default=100, help='')
...
#         # loss_semantic = contextual_loss(data_IR,F,0.5) + contextual_loss(data_VIS,F,0.5)
...
#         # 1 -> 1.2 -> 1.35
#         '''
#         exp1: loss_mse +  loss_in_grad + 1.5*lplssimB
#         exp4/exp6：loss_mse +  loss_in_grad + 1.5*lplssimB + lssimA     loss_in_grad = loss_in_grad*100
#         exp5/exp6：loss_mse +  loss_in_grad + 1.55*lplssimB + 1.1*(lssimA + lssimB) + lpB  对B图像的保留压过了A图像，考虑增加A图像的权重

#         test：  lossALL =  loss_mse +  loss_in_grad + 1.35*lplssimB + 1.3*lssimA + lssimB   A图像保留信息过少

#         lossALL =  loss_mse +  loss_in_grad + 1.45*lplssimB + 1.45*lssimA + 0.8*lssimB  A保留的信息过稍


#         lossALL =  loss_mse +  loss_in_grad + 1.8*lplssimB + lplssimA + 1.35*lssimA + 0.8*lssimB   图像b保持较少


#         lossALL =  loss_mse +  loss_in_grad + 8.60*lplssimB + 8*lplssimA   图像b保留过少

#         lossALL =  loss_mse +  loss_in_grad + 8.5*lplssimB + 8*lplssimA + 1.35*lssimA + 0.8*lssimB  B 保留过少
#         lossALL =  loss_mse +  loss_in_grad + 8.55*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB  B保留过少

#         lossALL =  loss_mse +  loss_in_grad + 8.65*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB  B保留太多了


#         ssALL =  loss_mse +  loss_in_grad + 8.57*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB   B几乎没有保留

#         lossALL =  loss_mse +  loss_in_grad + 8.6*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB  B保留过少

#         ossALL =  loss_mse +  loss_in_grad + 8.62*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB  B保留过多

#         lossALL =  loss_mse +  loss_in_grad + 8.615*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB B保留稍微过多

#         lossALL =  loss_mse +  loss_in_grad + 8.605*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB B保留过少

#         lossALL =  loss_mse +  loss_in_grad + 8.61*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB  B过少

#         lossALL =  loss_mse +  loss_in_grad + 8.613*lplssimB + 8*lplssimA + 1.35*lssimA + 0.9*lssimB  B过少

#         lossALL =  loss_mse +  loss_in_grad + 8.614*lplssimB + 8*lplssimA + 1.4*lssimA + lssimB  B过少

#         lossALL =  loss_mse +  loss_in_grad + 1.65*lplssimB + 1.5*lssimA + 0.8*lssimB   B过多

#         lossALL =  loss_mse +  loss_in_grad + 1.65*lplssimB + 1.5*lssimA + 0.8*lssimB 保底版本

#         -----
#         继续：
#         lossALL =  loss_mse +  loss_in_grad + 1.65*lplssimB + 1.6*lssimA + 0.8*lssimB

#         版本4：lossALL =  loss_mse +  loss_in_grad + 1.6*lplssimB + 1.65*lssimA + 0.8*lssimB

#         版本5：lossALL =  loss_mse +  loss_in_grad + 1.55*lplssimB + 1.65*lssimA + 0.8*lssimB

#         版本6：lossALL =  loss_mse +  loss_in_grad + 1.5*lplssimB + 1.7*lssimA + 0.8*lssimB

#         版本7： lossALL =  loss_mse +  loss_in_grad + 1.5*lplssimB + 0.5*lplssimA + 1.7*lssimA + 0.8*lssimB

#         版本8：lossALL =  loss_mse +  loss_in_grad + 1.5*lplssimB + 0.65*lplssimA + 1.7*lssimA + 0.8*lssimB
#         -----
#         '''
#         # lossALL =  loss_mse +  loss_in_grad + 1*lplssimB + 1.85*lssimA + 0.5*lssimB
#         lossALL =  loss_mse +  loss_in_grad + 1.63*lplssimB + 1.6*lssimA + 0.8*lssimB
...
for epoch in range(num_epochs):
    ...
    for i, (data_IR, data_VIS, text, index) in enumerate(trainloader):
        ...
        batchsize, channels, rows, columns = data_IR.shape
        weighttemp = int(np.sqrt(rows * columns))
        # print(criterion(image_in=data_IR, image_out=F, weight=weighttemp))
        lplssimA, lpA, lssimA = criterion(image_in=data_IR, image_out=F, weight=weighttemp)
        lplssimB, lpB, lssimB = criterion(image_in=data_VIS, image_out=F, weight=weighttemp)
        loss_in_grad, _, _ = loss(data_IR, data_VIS, F)
        loss_ssim = lplssimA + 0.996*lplssimB
        lossALL = loss_in_grad
        optimizer.zero_grad()
        lossALL.backward()
        optimizer.step()
