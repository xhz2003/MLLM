...
class ConvolutionalLayer(nn.Module):
    def __init__(self, input_channels=768, output_features=512):
        super(ConvolutionalLayer, self).__init__()
        self.conv = nn.Conv1d(in_channels=input_channels, out_channels=32, kernel_size=3, padding=1)
        self.pool = nn.AdaptiveAvgPool1d(1)  # Global average pooling to length 1
        self.fc = nn.Linear(32, output_features)  # Fully connected layer to output_features

    def forward(self, x):
        x = x.transpose(1, 2)  # Now x has shape [batch_size, embedding_dim, seq_len]
        x = self.conv(x)       # Apply convolution layer
...
class CLIPLoss(torch.nn.Module):
    def __init__(self):
        ...
        self.text_transform = ConvolutionalLayer(input_channels=768, output_features=512)

        # 【修改1】确保所有组件在初始化时移动到同一设备
        device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model = self.model.to(device)
        self.text_transform = self.text_transform.to(device)
    ...
    def forward(self, image_features, text_embeddings):
        ...
        # Convert text embeddings to text features with the new ConvolutionalLayer block
        # text_embeddings: [batch_size, seq_len, embedding_dim]
        text_features = self.text_transform(text_embeddings)  # Use the ConvolutionalLayer for transformation
        image_features = image_features.mean(dim=[2, 3])  # 压缩空间维度
        similarity = 1 - abs(torch.cosine_similarity(image_features, text_features).mean())
        return similarity.float() if similarity.float() > 0.2 else 0.0
