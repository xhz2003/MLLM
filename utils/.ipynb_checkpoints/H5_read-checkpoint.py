# import torch
# from torch.utils.data import Dataset
# import h5py

# class H5ImageTextDataset(Dataset):
#     def __init__(self, h5_file_path):
#         self.h5_file_path = h5_file_path

#     def __len__(self):
#         with h5py.File(self.h5_file_path, 'r') as h5_file:
#             return len(h5_file['imageA'])

#     def __getitem__(self, idx):
#         with h5py.File(self.h5_file_path, 'r') as h5_file:
#             group_names = list(h5_file.keys())
#             sample_name = list(h5_file[group_names[0]].keys())[idx]
#             imageA = torch.from_numpy(h5_file['imageA'][sample_name][()])
#             imageB = torch.from_numpy(h5_file['imageB'][sample_name][()])
#             text = torch.from_numpy(h5_file['text'][sample_name][()])
#             return imageA, imageB, text, sample_name

import torch
from torch.utils.data import Dataset
import h5py

class H5ImageTextDataset(Dataset):
    def __init__(self, h5_file_path, max_samples=150):
        self.h5_file_path = h5_file_path
        self.max_samples = max_samples  # 最大样本数限制
        # 预先获取实际数据量，避免每次调用__len__都打开文件
        with h5py.File(self.h5_file_path, 'r') as h5_file:
            self.actual_length = len(h5_file['imageA'])
        # 确定实际使用的样本数（取实际数量和最大限制中的较小值）
        self.effective_length = min(self.actual_length, self.max_samples)

    def __len__(self):
        return self.effective_length  # 返回限制后的样本数

    def __getitem__(self, idx):
        # 确保索引不超过限制
        if idx >= self.effective_length:
            raise IndexError("Index out of range")
            
        with h5py.File(self.h5_file_path, 'r') as h5_file:
            group_names = list(h5_file.keys())
            sample_name = list(h5_file[group_names[0]].keys())[idx]
            imageA = torch.from_numpy(h5_file['imageA'][sample_name][()])
            imageB = torch.from_numpy(h5_file['imageB'][sample_name][()])
            text = torch.from_numpy(h5_file['text'][sample_name][()])
            return imageA, imageB, text, sample_name
    